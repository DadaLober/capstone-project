...
const FormComp: React.FC<MarkerFormProps> = ({ selectedPosition, address, setAddress }) => {
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        const data: FormData = {
            address ,
            latitude: selectedPosition ? selectedPosition.lat : 0,
            longitude: selectedPosition ? selectedPosition.lng : 0,
            size: (event.target as any).size.value,
            price: (event.target as any).price.value,
            attributes: additionalFields.map((_, index) => (event.target as any)[`field-${index}`].value),
            files: uploadedFiles,
        };
